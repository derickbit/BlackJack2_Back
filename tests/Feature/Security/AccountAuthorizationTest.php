...
class AccountAuthorizationTest extends SecurityTestCase
{
    ...
    public function test_owner_can_delete_own_account(): void
    {
        $owner = User::factory()->create();
        Sanctum::actingAs($owner);
        $this->deleteJson('/api/users/'.$owner->id)->assertOk();
    ...
    public function test_public_profiles_only_expose_id_and_name(): void
    {
        $owner = User::factory()->create();
        $this->getJson('/api/users')->assertOk()->assertExactJson([
            'data' => [['id' => $owner->id, 'name' => $owner->name]],
        ]);
        $this->getJson('/api/users/'.$owner->id)->assertOk()->assertExactJson([
            'data' => ['id' => $owner->id, 'name' => $owner->name],
        ]);
        $serialized = $owner->toArray();
        foreach (['password', 'remember_token', 'email', 'email_verified_at'] as $field) {
    ...
    public function test_public_registration_still_returns_own_email_without_password(): void
    {
        Notification::fake();
        $response = $this->postJson('/api/users', $this->payload() + ['role' => 'admin']);
        $response->assertCreated()->assertJsonPath('data.email', 'updated@example.test');
        $this->assertArrayNotHasKey('password', $response->json('data'));
        $this->assertFalse(User::where('email', 'updated@example.test')->firstOrFail()->isAdmin());
    }
